...
new_sand = True
settled_sand = 0
sand_source = tuple(sand_source)

while True:
    if (new_sand):
        new_sand = False
        sand_position = list(sand_source)
    if ((len(rock_graph) <= sand_position[1] + 1)):
        #print(f"{sand_position} // {sand_source}")
        break
    elif ((rock_graph[sand_position[1] + 1][sand_position[0]] == ".") and (len(rock_graph) > sand_position[1] + 1)):
        sand_position[1] += 1
    elif ((rock_graph[sand_position[1] + 1][sand_position[0] - 1] == ".") and (len(rock_graph) > sand_position[1]) and (0 < sand_position[0])):
        sand_position[1] += 1
        sand_position[0] -= 1
    elif ((rock_graph[sand_position[1] + 1][sand_position[0] + 1] == ".") and (len(rock_graph) > sand_position[1]) and (len(rock_graph[0]) > sand_position[0])):
        sand_position[1] += 1
        sand_position[0] += 1
    else:
